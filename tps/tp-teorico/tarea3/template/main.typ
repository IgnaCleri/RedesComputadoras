...
#show: informe.with(
  ...
  resumen: [*_Objetivo_ --- Resolver las preguntas de repaso y los problemas del Capítulo 4 de @stallings2004, sobre medios de transmisión guiados e inalámbricos, antenas y propagación.*],
...
// Las preguntas y problemas se numeran como en el libro, en el texto del título.
#set heading(numbering: none)

// Coma decimal: Typst agrega un espacio después de la coma en modo matemático.
...
$ (4 pi A_e)/lambda^2 = (4 pi f^2 A_e)/c^2 $
...
Y el área efectiva de una antena parabólica de área $A$ será $0,56 A$, siendo la ganancia en potencia igual a $(7 A)/lambda^2$
...
Supóngase que unos datos se almacenan en disquetes de $1,4 "Mb"$ que pesan $30 "g"$ cada uno y que una compañía aérea transporta $10^4 "kg"$ de disquetes a una velocidad de $1.000 "km/h"$ sobre una distancia de $5.000 "km"$. ¿Cuál es la velocidad de transmisión en bits por segundo de este sistema?

=== Respuesta

Primero tenemos que que ver cuantos disquetes se pueden tranportar en cada viaje:

$ (30 "g")/((10^4 dot 1000) "g") = 333333.333 $

Entonces por cada viaje viajan:

$ "Total de bytes" = 333.333,33 "disquetes" times (1,4 times 10^6 "bytes/disquete") approx 4,66667 times 10^11 "bytes" $

En bits eso seria:

$ "Total de bits" = 4,66667 times 10^11 "bytes" times 8 "bits/byte" approx 3,73333 times 10^12 "bits" $

Ahora el tiempo que le toma el avion para recorrer esa distancia es:

$ "Tiempo " (t) = (5.000 "km")/(1.000 "km/h") = 5 "horas" $

La velocidad de transmision en $"bps"$ es:

$ "Velocidad" = (3,73333 times 10^12 "bits")/(5 "h" dot 3600 "s/h") approx 207.407.407 "bps" $
...
$ lambda = c/f => lambda = (3 times 10^8 "m/s")/(30 "Hz") = 10.000.000 "m" = 10.000 "Km" $
...
$ lambda = c/f => lambda = (3 times 10^8 "m/s")/(300 "Hz") = 1.000.000 "m" = 1.000 "Km" $
...
$ "Longitud de la antena" = lambda/2 => 2 dot "Longitud de la antena" = lambda $

Entonces $"Longitud de la antena" = 1 "m"$:

$ 2 dot 1 = lambda => lambda = 2 $

Ahora despejamos la frencuencia de:

$ lambda = c/f => 2 "m" = (3 times 10^8 "m/s")/f => f = (3 times 10^8 "m/s")/(2 "m") = 150 "Mhz" $
...
- Velocidad de propagacion = $3 times 10^8 "m"/"s"$

$ L = lambda/2 => lambda = 2 dot 0.0025 "m" = 0.005 "m" $

Luego, sabiendo que:

$ c = lambda dot f => f = lambda/f = (3 times 10^8 "m"/"s")/(0.005 "m") = 6 times 10^10 "Hz" $

La frecuencia que recibiraa travez de el empaste es de $60 times 10^10 "Hz"$
...
La potencia recibida ($P_r$) en una comunicación en espacio libre expresada en función de las áreas efectivas de las antenas transmisora y receptora ($A_(e t)$ y $A_(e r)$) es:

$ P_r = (P_t dot A_(e t) dot A_(e r))/(d^2 dot lambda^2) $

Luego: $lambda = c/f$, sustituyendo:

$ P_r = (P_t dot A_(e t) dot A_(e r) dot f^2)/(d^2 dot c^2) $

- Opción 1

  Para

  $ f_1 = 2f => P_r = (P_t dot A_(e t) dot A_(e r) dot (2f)^2)/(d^2 dot c^2) $
  $ P_r = (P_t dot A_(e t) dot A_(e r) dot 4 f^2)/(d^2 dot c^2) $
  $ P_r = 4 dot (P_t dot A_(e t) dot A_(e r) dot f^2)/(d^2 dot c^2) $

- Opción 2

  $ A'_(e t) = 2 A_(e t) quad "y" quad A'_(e r) = 2 A_(e r) $

  Luego

  $ P_r = (P_t dot 2 A_(e t) dot 2 A_(e r) dot (f)^2)/(d^2 dot c^2) $
  $ P_r = 4 dot (P_t dot A_(e t) dot A_(e r) dot f^2)/(d^2 dot c^2) $
...
Eso en dB se traduce a $10 log_10 (4) approx 6 "dB"$.
...
#align(center, image("assets/parabola-de-reflexion.png", width: 55%))
...
$ (dif)/(dif x) y^2 &= (dif)/(dif x) 2 p x \
2 y (dif y)/(dif x) (x) &= 2 p \
(dif y)/(dif x) (x) &= p/y $
...
$ (dif y)/(dif x) (x_1) = p/y_1 $

luego

$ tan beta = (dif y)/(dif x) (x_1) = p/y_1 $
...
La pendiente de $P F$ es $m_(P F) = y_1/(x_1 - p\/2)$. Despejando $x_1$ en la ecuación de la parábola tenemos que:

$ x_1 = y_1^2/(2 p) => m_(P F) = y_1/(y_1^2/(2 p) - p/2) = (2 p y_1)/(y_1^2 - p^2) \
tan(alpha_1) = m_(P F) = (2 p y_1)/(y_1^2 - p^2) $

Luego, aplicando la propiedad sugerida:

$ tan(alpha) &= tan(alpha_1 - beta) \
             &= (tan(alpha_1) - tan(beta))/(1 + tan(alpha_1) dot tan(beta)) \
             &= ((2 p y_1)/(y_1^2 - p^2) - p/y_1)/(1 + ((2 p y_1)/(y_1^2 - p^2))(p/y_1)) \
             &= ((2 p y_1^2 - p(y_1^2 - p^2))/(y_1 (y_1^2 - p^2)))/(1 + (2 p^2)/(y_1^2 - p^2)) \
             &= ((p y_1^2 + p^3)/(y_1 (y_1^2 - p^2)))/(((y_1^2 - p^2) + 2 p^2)/(y_1^2 - p^2)) \
...
$ P = 0,1 "W", quad f = 2 "GHz" quad "y" quad "diámetro:" D = 1,2 "m" $

$ bold("Área: ") A = pi times (D/2)^2 = pi times ((1,2)/2)^2 = 1,13 "m"^2 $
...
$ bold("Potencia efectiva radiada: ") "EIRP" ("dBm") = P_t ("dBm") + G_t ("dB") = bold("45,45 dBm") $
...
#figure(
  image("assets/linea-de-vision.jpg", width: 60%),
...
$ d = sqrt(2 dot 6.370 dot h/1000) = sqrt("12,74") dot sqrt(h) approx 3,57 sqrt(h) $
...
Con n#sub[1] = 1,0003 (aire) y n#sub[2] = 4/3 (agua):
