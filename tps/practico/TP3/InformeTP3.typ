// barcala 0.3.0 tiene un bug: si un autor no tiene email, el post-transform de
// `email` explota (llama a `.split` sobre `none`). _vendor/barcala/ es una copia
// local con ese único método parcheado (ver _vendor/barcala/types.typ), necesaria
// porque este informe no muestra emails en la carátula.
#import "_vendor/barcala/lib.typ": informe
...
#show: informe.with(
  ...
  autores: (
    (nombre: "Viberti, Benjamin", legajo: "DNI 46224179"),
    (nombre: "Espinoza Sutta, Aaron Alejandro", legajo: "DNI 96009173"),
    (nombre: "Cleri, Juan Ignacio", legajo: "DNI 46452662"),
    (nombre: "Pineda, Juan Ignacio", legajo: "DNI 45591343"),
    (nombre: "Grafión, Atilio Leonel", legajo: "DNI 43940195"),
    (nombre: "Badenes, Tomás", legajo: "DNI 44785038"),
    (nombre: "Oviedo, Ignacio Nicolas", legajo: "DNI 43940195"),
    (nombre: "Mendez, Jorge Nicolas", legajo: "DNI 41301342"),
  ),
  titulo: [Direccionamiento y conexiones: Ethernet, IP y TCP],
  fecha: datetime(year: 2026, month: 9, day: 10),
)

// Enlaces de colores
#show link: set text(blue)

// Estilo uniforme de tablas
...
#figure(
  {
    set text(size: 9pt)
    table(
      columns: (3.6cm, 1.8cm, 6.8cm, 1fr),
      table.header([*Name*], [*DNI*], [*Mail UNC*], [*Github*]),
      [Viberti, Benjamin], [46224179], [b.viberti\@mi.unc.edu.ar], [#link("https://github.com/benjaviberti")[\@benjaviberti]],
      [Espinoza Sutta, Aaron Alejandro], [96009173], [aaron.espinoza_4500\@mi.unc.edu.ar], [#link("https://github.com/Aaron45000")[\@Aaron45000]],
      [Cleri, Juan Ignacio], [46452662], [ignacio.cleri\@mi.unc.edu.ar], [#link("https://github.com/IgnaCleri")[\@IgnaCleri]],
      [Pineda, Juan Ignacio], [45591343], [juan.ignacio.pineda\@mi.unc.edu.ar], [#link("https://github.com/juanignaciopineda-dot")[\@juanignaciopineda-dot]],
      [Grafión, Atilio Leonel], [43940195], [atilio.grafion\@mi.unc.edu.ar], [#link("https://github.com/Aollgn")[\@Aollgn]],
      [Badenes, Tomás], [44785038], [tomasbadenes\@mi.unc.edu.ar], [#link("https://github.com/b-Tomas")[\@b-Tomas]],
      [Oviedo, Ignacio Nicolas], [43940195], [ignacio.oviedo.239\@mi.unc.edu.ar], [#link("https://github.com/GIX02")[\@GIX02]],
      [Mendez, Jorge Nicolas], [41301342], [jorge.mendez\@mi.unc.edu.ar], [#link("https://github.com/jorge088")[\@jorge088]],
    )
  },
  caption: [Integrantes del grupo],
)
...
- *Detección y control de errores*: su servicio principal. Si el protocolo de enlace funciona correctamente, la capa superior puede asumir que la transmisión sobre _ese_ enlace está libre de errores.
...
Que es una trama ethernet? Una trama Ethernet es la unidad básica de datos (PDU) que se envía a través de una red local (LAN) en la capa de enlace de datos. Es el que empaqueta los datos reales para llevarlos de una tarjeta de red a otra mediante direcciones físicas MAC.

#figure(
  table(
    columns: (auto, auto, 1fr),
    table.header([*Campo*], [*Tamaño*], [*Funcion*]),
  ...
  caption: [Campos de una trama Ethernet],
)
...
#figure(
  image("imagenes/image-2.png", width: 85%),
  caption: [Paquetes TLS capturados, filtrados con `tls`],
)
...
#figure(
  image("imagenes/image-3.png", width: 85%),
  caption: [Detalle del paquete 35: capas Ethernet II, IPv4 y TCP],
)
...
#figure(
  table(
    ...
    table.header([*Campo*], [*Dirección MAC*], [*Fabricante (OUI)*]),
    [*Source*], [`58:11:22:48:01:66`], [ASUSTek Computer],
    [*Destination*], [`f0:81:75:35:a4:4f`], [Sagemcom Broadband],
  ),
  caption: [Direcciones MAC de la trama Ethernet II],
)

Wireshark resuelve automáticamente los primeros tres bytes de cada dirección MAC (el OUI, _Organizationally Unique Identifier_) contra el fabricante registrado de la tarjeta de red, lo que da una pista directa sobre a qué dispositivo pertenece cada una:
...
#figure(
  ...
  caption: [Comparación entre direcciones MAC e IP de la misma trama],
)
...
Si los dos extremos emiten SYN cruzados, no se produce ningún problema, ambos lados responden con SYN/ACK
...
#figure(
  ...
  caption: [Instancia del servidor packet sender],
)

#figure(
  table(
    columns: (auto, auto),
    table.header([*Extremo*], [*Puerto*]),
    [Cliente], [61414],
    [Servidor], [61373],
  ),
...
#figure(
  image("imagenes/wireshark_dstport.jpg", width: 80%),
...
#figure(
  image("imagenes/wireshark_port.jpg", width: 80%),
  caption: [Filtro `tcp.port == 61373`],
)
...
#figure(
  table(
    columns: (auto, 1fr),
    table.header([*Paquete*], [*Significado*]),
  ...
  caption: [Paquetes del three-way handshake capturados],
)
...
#figure(
  ...
  caption: [Paquetes del intercambio de datos],
)

#figure(
  image("imagenes/wireshark_paq_256.jpg", width: 80%),
  caption: [Paquete 256 en Wireshark],
)
...
#figure(
  image("imagenes/wireshark_paq_256_close.jpg", width: 80%),
  caption: [Four-way handshake de cierre],
)
...
#figure(
  ...
  caption: [Paquetes del four-way handshake capturados],
)
...
#figure(
  image("imagenes/wireshark_four_way_handshake_piggyback.png", width: 80%),
  caption: [Four-way handshake con piggyback],
)
...
Se levantó una instancia de *PacketSender* en modo cliente y se estableció una conexión TCP contra el servidor, utilizando la opción *Persistent TCP* para mantener la sesión abierta durante todo el intercambio:
...
#figure(
  image("imagenes/image.png", width: 80%),
  caption: [Comunicación con el servidor],
)

#figure(
  table(
    columns: (auto, auto),
    table.header([*Comando enviado*], [*Respuesta del servidor*]),
    [`hola\r`], [`hola :)\n`],
    [`ping\r`], [`pong\n`],
    [`wireguardians\r`], [`seq: 25, payload: cQ\n`],
  ),
  caption: [Comandos enviados y respuestas del servidor],
)

El comando correspondiente a nuestro grupo (*wireguardians*) fue reconocido como comando válido por el servidor, que respondió con el par `seq: 25, payload: cQ`: el número de secuencia que nos corresponde dentro del conjunto de grupos y el fragmento de información que nos tocó.

== Captura de la sesión con Wireshark

#figure(
  image("imagenes/image-1.png", width: 80%),
  caption: [Captura de paquetes en Wireshark],
)

La captura se filtró con `tcp.port == 5555 or tcp.port == 40527` para aislar únicamente la conversación con el servidor. Sobre los paquetes capturados se puede observar:

- El intercambio sigue el patrón *pedido / respuesta / confirmación*: por cada comando el cliente envía un segmento `[PSH, ACK]`, el servidor contesta con otro `[PSH, ACK]` que transporta la respuesta y el cliente cierra el ciclo con un `[ACK]` sin datos (`Len=0`).
- Los números de secuencia y de acuse avanzan exactamente en la cantidad de bytes transportados: `Seq=1 Len=5` (`hola\r`) → el servidor responde con `Ack=6` y `Len=8` (`hola :)\n`), y así sucesivamente hasta `Seq=25 Ack=35`
- El detalle del _frame_ 182 muestra el encapsulamiento completo de las capas *Ethernet II* (Src `00:e0:4c:68:0e:87`) → *IPv4* (Src `192.168.0.106`, Dst `34.136.251.235`) → *TCP* (Src Port `40527`, Dst Port `5555`) → *Data (5 bytes)*.
- En el panel de bytes se lee la carga útil en claro: `hola\r`.

La captura completa se encuentra en #link("Captura%20de%20Paquetes/Ejercicio4-CapturaPaquetes.pcapng")[`Captura de Paquetes/Ejercicio4-CapturaPaquetes.pcapng`] (interfaz `enp3s0f3u1u1`, 3789 paquetes registrados a lo largo de 191 s). De ese total, el filtro aísla *16 paquetes* que constituyen la totalidad de la conexión con el servidor, desde su apertura hasta su cierre.
...
#figure(
  {
    set text(size: 8pt)
    table(
      columns: (1.1cm, 1.4cm, 3.6cm, 1.8cm, 1cm, 1cm, 1cm, 1.3cm, 1fr),
      table.header(
  ...
  caption: [Detalle de los 16 paquetes de la conversación con el servidor],
)
...
+ *SYN* (75): el cliente propone la conexión desde el puerto 40527
+ *SYN, ACK* (78): envía su propio número de secuencia inicial y acusa el del cliente con `Ack=1`.
+ *ACK* (79): el cliente confirma. A partir de aquí la conexión está establecida y ambos extremos pueden enviar datos.

Nótese que los dos primeros segmentos miden 74 bytes y llevan cabecera TCP de *40 bytes* (20 fijos + 20 de opciones), mientras que del `ACK` en adelante la cabecera baja a *32 bytes*: las opciones de negociación sólo se transmiten en el handshake.

=== Cierre de la conexión: four-way handshake

Al cerrar la ventana de chat de PacketSender se capturó el cierre:

+ *FIN, ACK* (3113): el cliente indica que no tiene más datos para enviar.
+ *FIN, ACK* (3119): el servidor acusa el FIN del cliente (`Ack=26`) y, en el mismo segmento, envía su propio FIN.
+ *ACK* (3120): el cliente acusa el FIN del servidor (`Ack=36`) y la conexión queda cerrada.

Aunque el cierre son cuatro pasos (`FIN` → `ACK` → `FIN` → `ACK`), en la práctica se observan *sólo tres segmentos*: el servidor fusionó su `ACK` con su propio `FIN` en un único paquete, optimización habitual cuando el extremo pasivo tampoco tiene datos pendientes.
...
#link("https://www.youtube.com/watch?v=dQw4w9WgXcQ")[Link de Resultado nada sospechoso]
...
#figure(
  {
    ...
    table(
      ...
      table.header([*Nombre*], [*Seq*], [*Payload*]),
      [\#hiddenSSID], [1], [ht],
      [Auracast], [2], [tp],
      [BitBros], [3], [s:],
      [Bitless], [4], [`//`],
  ...
  caption: [Fragmentos de payload recibidos por cada grupo],
)
